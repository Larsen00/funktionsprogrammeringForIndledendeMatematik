...
\begin{lstlisting}[
    language={FSharp}, 
    label={number_type}, 
    caption={Typen for \texttt{\texttt{Number}}}
    ]
    type \texttt{Number} = | Int of int | Rational of rational | Complex of complex
\end{lstlisting}
...
\texttt{Expr<'a>}  typen er dermed en polymorfisk type, hvor 'a er typen for den tal mængde hvor vi kan lave brugerdefinerede matematiske operationer. Et exemplar på en \texttt{Expr<Number>}  er givet i Listing \ref{lst:expr_example}. Her ses det at når udtrykket $-(3 \cdot x - 5) \cdot 2$ visualiseres er det i prefix notation. 
